...
# =========================================================
# BOT COMPRASNET / CONTRATOS GOV - VERSAO RAPIDA (HTTP DIRETO)
# =========================================================
# Igual ao "Bot comprasnet .py", mas troca a extração do detalhe de
# cada ITEM: em vez de abrir uma aba e navegar no Chrome pra cada um
# (lento: renderiza HTML/CSS/JS/fontes por item), busca o HTML de
# cada item via requisição HTTP direta (reaproveitando os cookies da
# sessão já logada no Chrome) e faz o parsing com BeautifulSoup — e
# dispara várias dessas requisições em paralelo.
#
# A listagem de pregões e de itens continua via Playwright (usa
# DataTables/paginação client-side, não vale a pena reescrever isso
# em HTTP puro). O ganho está só na fase que domina o tempo total:
# o detalhe item a item.
# =========================================================
...
HTTP_CONCORRENCIA = 6  # requisições simultâneas de detalhe de item
...
# =========================================================
# GOOGLE SHEETS COM RETRY / CHROME
# =========================================================
# update_com_retry / clear_com_retry / abrir_chrome -> comum.py
...
# =========================================================
# EXTRAÇÃO GENÉRICA - VIA PLAYWRIGHT (páginas de listagem/pregão)
# =========================================================
...
# =========================================================
# EXTRAÇÃO GENÉRICA - VIA HTML PURO (BeautifulSoup, detalhe de item)
# =========================================================

def extrair_por_rotulos_html(soup: BeautifulSoup, rotulos: list[str]) -> dict:
    dados = {}
    ...
    for row in soup.select("table tr"):
        ...
        chave = normalizar_espacos(cells[0].get_text()).replace(":", "")
        valor = normalizar_espacos(cells[1].get_text())
...
def extrair_ata_fornecedor_html(soup: BeautifulSoup) -> dict:
    resultado = {"numero_ata": "", "fornecedor": ""}
    ...
    for tabela in soup.find_all("table"):
        ...
        header_cells = linhas[0].find_all(["th", "td"])
        headers = [normalizar_espacos(c.get_text()) for c in header_cells]
        ...
        idx_numero = next(i for i, h in enumerate(headers_norm) if h == "número" or h == "numero")
        idx_fornecedor = next(i for i, h in enumerate(headers_norm) if "fornecedor" in h)

        for linha in linhas[1:]:
            cels = linha.find_all(["th", "td"])

            if len(cels) <= max(idx_numero, idx_fornecedor):
                continue

            numero_ata = normalizar_espacos(cels[idx_numero].get_text())
            fornecedor = normalizar_espacos(cels[idx_fornecedor].get_text())
...
def extrair_quantidade_valor_unitario_html(soup: BeautifulSoup) -> dict:
    resultado = {"quantidade": "", "valor_unitario": "", "vig_inicio": "", "vig_fim": ""}
    ...
    for tabela in soup.find_all("table"):
        ...
        idx_ini = next(i for i, h in enumerate(headers_norm) if "vigência início" in h or "vigencia inicio" in h)
        idx_fim = next(i for i, h in enumerate(headers_norm) if "vigência fim" in h or "vigencia fim" in h)
        idx_qtd = next(i for i, h in enumerate(headers_norm) if "quantidade" in h)
        idx_vu = next(i for i, h in enumerate(headers_norm) if "valor unitário" in h or "valor unitario" in h)
        ...
        for linha in linhas[1:]:
            ...
            resultado["vig_inicio"] = normalizar_espacos(cels[idx_ini].get_text())
            resultado["vig_fim"] = normalizar_espacos(cels[idx_fim].get_text())
            resultado["quantidade"] = normalizar_espacos(cels[idx_qtd].get_text())
            resultado["valor_unitario"] = normalizar_espacos(cels[idx_vu].get_text())
...
def pegar_qtds_pmb_html(soup: BeautifulSoup) -> dict:
    resultado = {"qtd_autorizada": "0", "qtd_saldo": "0"}
    ...
    for tabela in soup.find_all("table"):
        ...
        idx_unidade = next(i for i, h in enumerate(headers_norm) if "unidade" in h)
        idx_aut = next(i for i, h in enumerate(headers_norm) if "qtd autorizada" in h)
        idx_saldo = next(i for i, h in enumerate(headers_norm) if "qtd saldo" in h)
        ...
        for linha in linhas[1:]:
            ...
            unidade = normalizar_espacos(cels[idx_unidade].get_text())

            if UNIDADE_PMB in unidade:
                resultado["qtd_autorizada"] = normalizar_espacos(cels[idx_aut].get_text())
                resultado["qtd_saldo"] = normalizar_espacos(cels[idx_saldo].get_text())
                return resultado
...
# =========================================================
# PORTAL - DETALHE DO ITEM VIA HTTP (rápido, em paralelo)
# =========================================================

async def extrair_item_via_http(client: httpx.AsyncClient, sem: asyncio.Semaphore, item_info: dict):
    """Busca e faz o parsing do detalhe de UM item via HTTP.
    Retorna (item_dict, None) em caso de sucesso, ou (None, mensagem_erro)."""
    url = item_info["detalhe_url"]
    ...
    if "/login" in str(resp.url):
        return None, "Sessão expirada (redirecionado para login)"
    ...
    dados = extrair_por_rotulos_html(
        soup,
        ["Número", "Tipo Item", "Descrição", "Descrição detalhada"]
    )

    ata_fornecedor = extrair_ata_fornecedor_html(soup)
    qtd_valor = extrair_quantidade_valor_unitario_html(soup)
    qtds_pmb = pegar_qtds_pmb_html(soup)
    ...
    item = {
        ...
        "Link detalhe item": url,
    }

    return item, None


async def coletar_itens_via_http(page, links_itens: list[dict]) -> list:
    """Busca o detalhe de todos os itens de `links_itens` via HTTP direto,
    em paralelo (limitado por HTTP_CONCORRENCIA), reaproveitando os
    cookies da sessão logada no Chrome (via CDP). Retorna uma lista de
    tuplas (item|None, erro|None) na mesma ordem de `links_itens`."""
    ...
    async with httpx.AsyncClient(cookies=cookies, headers=headers, follow_redirects=True) as client:
        tarefas = [extrair_item_via_http(client, sem, item_info) for item_info in links_itens]
        return await asyncio.gather(*tarefas)


# =========================================================
# PORTAL - DETALHE DO PREGÃO (via Playwright, baixo volume)
# =========================================================
...
async def processar_pregao(page, planilha, reg: dict):
    ...
    if reg.get("itens_url"):
        ...
        if links_itens:
            log(f"Buscando detalhe de {len(links_itens)} item(ns) via HTTP (concorrência={HTTP_CONCORRENCIA})...")
            resultados = await coletar_itens_via_http(page, links_itens)

            for j, (item_info, (item, erro)) in enumerate(zip(links_itens, resultados), start=1):
                item_ref = item_info.get("numero_lista", "")

                if erro:
                    log(f"  ERRO no item {j}/{len(links_itens)} ({item_ref}): {erro}")
                    registrar_erro(planilha, "ITEM_ERRO", numero_ano, item_ref, erro)
                    erros_extracao += 1
                    continue
            ...
            log(f"  {len(itens)}/{len(links_itens)} item(ns) extraído(s) com sucesso.")
        ...
        async with async_playwright() as p:
            ...
            for i, reg in enumerate(registros_filtrados, start=1):
                try:
                    ...
                    if resultado:
                        registros_indice.append(resultado)
